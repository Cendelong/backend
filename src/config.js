...
/**
 * 获取 CORS 允许源列表
 * v6.9.15修复：未配置FRONTEND_URL时返回['*']而非null，
 * 否则 CORS 回调中 null.includes() 会抛 TypeError，导致所有跨域请求失败
 * （账号列表无法读取Render云端账号的根因）
 */
export function getAllowedOrigins() {
  ...
  if (!config.frontendUrls.length) return ['*']; // 未配置 → 允许所有（开发模式）
